...
const platformIcons = [
  { src: MEDIA.platformWindows, label: 'Windows' },
  { src: MEDIA.platformMacos, label: 'macOS' },
  { src: MEDIA.platformLinux, label: 'Linux' },
] as const;
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`} aria-label="Desktop platforms">
      {platformIcons.map((platform) => (
        <span
          key={platform.src}
          className="inline-flex items-center justify-center rounded-sm bg-white/5 border border-white/10 p-1.5"
        >
          <img
            src={platform.src}
            alt=""
